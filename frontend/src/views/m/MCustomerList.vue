...
async function onLoad() {
  try {
    const resp: any = await listCustomers({ page: page.value, page_size: pageSize })
  ...
  <div class="m-list">
    ...
    >
      <van-cell
        ...
        :key="item.id"
        :title="item.name"
        :label="`联系人：${item.contact_person || '—'}　业务员：${item.salesman_name || '—'}`"
      />
    </van-list>
    <van-empty v-if="finished && !rows.length" description="暂无客户数据" />
  </div>
</template>
