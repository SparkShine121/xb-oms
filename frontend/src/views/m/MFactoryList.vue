...
async function onLoad() {
  try {
    const resp: any = await listFactories({ page: page.value, page_size: pageSize })
  ...
  <div class="m-list">
    ...
    >
      <van-cell
        ...
        :key="item.id"
        :title="item.name"
        :label="`别名：${item.alias || '—'}　联系人：${item.contact || '—'}`"
      />
    </van-list>
    <van-empty v-if="finished && !rows.length" description="暂无工厂数据" />
  </div>
</template>
